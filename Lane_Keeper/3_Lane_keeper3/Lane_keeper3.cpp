...
void cal_lane_position(Mat& img, const vector < float >& L_point, const int line_no, const float* lane_pos);
...
int main(void) {
    ...
    while (true) {       
        ...
        cal_lane_position(mat_image_org_color, intersect_base, linesP.size(), lane_pos);
...
void cal_lane_position(Mat& img, const vector < float >& L_point, const int line_no, const float* lane_pos) {
    ...
    vector < int > label(line_no, -1);
    vector < int > cluster_size(line_no);
    vector < float > cluster_center(line_no);
    int label_no = 0;
    int no_cnt = 0;
    label[0] = label_no + 1;
        ...
        for (j = i, no_cnt = 0; j < line_no; j++) {
            if (label[j] == -1) {
                ...
                if (distance[i] >= LINE_WIDTH - LINE_WIDTH_TOL && distance[i] <= LINE_WIDTH + LINE_WIDTH_TOL) {
                    ...
                    cluster_size[label[j]]++;
                }
                else {
                    no_cnt += 1;
                    // printf("no_cnt : %2d\n", no_cnt);
                }
                // printf("Error[%d %d] = %3.1f\n", i, j, distance[i]);
            }
        }
    ...
    for (i = 0; i < line_no; i++) {
        if (label[i] != -1) {
            cluster_center[label[i]] += L_point[i];
        }
    }
    ...
    for (i = 0; i < line_no; i++) {
        if (cluster_size[i] > 0) {
            cluster_center[i] /= cluster_size[i] + 1;
            assist += cluster_center[i];
            cluster_cnt++;
            circle(img,
                Point(cluster_center[i], ASSIST_BASE_LINE),
                5, RED, -1);

            printf("cluster_center[%d] : %7.3f\n", i, cluster_center[i]);
        }
    }
    ...
    if (cluster_cnt > 0) {
        assist /= cluster_cnt;
        line(img,
            Point(assist, ASSIST_BASE_LINE - guide_height1 * 2.5),
            Point(assist, ASSIST_BASE_LINE + guide_height1 * 2.5),
            RED, 4, 0);

        printf("Assist center : %7.3f\n", assist);
    }
}
